...
# ============================================================
@paciente_bp.route("/validar_codigo", methods=["GET", "POST"])
def validar_codigo():
    telefono = session.get('telefono_temp')
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            ...
            WHERE telefono=%s ORDER BY expiracion DESC LIMIT 1
        """, (telefono,))
        fila = cursor.fetchone()

        if fila and fila["codigo"] == codigo_ingresado:
            # Código correcto, cargar ID del paciente
            cursor.execute("SELECT idPaciente, NombreCompleto FROM paciente WHERE telefono=%s", (telefono,))
            paciente = cursor.fetchone()
            ...
            session.pop("telefono_temp", None)

            print("Código correcto. ID Paciente en sesión:", session["idPaciente"])
            print("Flujo:", flujo)
            
            # 🔥 Si viene del LOGIN → llevarlo al calendario
            if flujo == "login":
                return redirect("/paciente/menu_citas")
            
            return redirect("/menu/paciente")

        return render_template("validar_codigo.html", mensaje="Código incorrecto")
...
def alta_paciente():
    ...
    if request.method == "POST":
        ...
        if not fila or fila["codigo"] != codigo or datetime.now() > fila["expiracion"]:
            mensaje = "❌ Código WhatsApp inválido o expirado."
            tipo_mensaje = "error"
            return render_template("alta_paciente.html", mensaje=mensaje, tipo_mensaje=tipo_mensaje)
